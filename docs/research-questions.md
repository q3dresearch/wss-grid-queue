...
| Q2 | How many approved projects are already late? | nothing | **answered: 228 of 3,041 live projects (7%) are past their Expected ISD, carrying $1.0B. Median 0.7 years over, worst 6.7** |
...
## A parsing trap, found while screening

**Membership in a file is not the project's status.** 169 rows in the *Approved*
file and 12 in *Under Evaluation* already carry `M4 - Project in Service`. The
files lag their own `Planning Status` column, so any count that trusts the
filename is wrong — reading "3,211 approved projects" off the Approved file
overstates the live pipeline by 5%. Parse the status column.
